...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;)
	{
		...
		// Increment the number of exits from vm by 1
		numOfExits += 1;
		...
		case KVM_EXIT_IO:
			...
			// If IO Direction is OUT, Increment ioOut var
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT)
				ioOut++;
			// If IO Direction is IN, Increment ioIn var
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN)
				ioIn++;
			// printf("THIS IS INSIDE KVM:: IO_IN:%d, IO_OUT:%d\n",ioIn,ioOut);
			// IO Port 0xE9 - HC_print8bit
			...
			// IO Port 0xE5 - HC_numExitsByType
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xE5)
			{
				// Make the string which is need to be sent
				char str[50];
				sprintf(str, "IO in: %u\nIO out: %u\n", ioIn, ioOut);
				// Get the io memory to write
				uint32_t *mem = (uint32_t *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
				// Write the string
				strncpy((char *)vm->mem + *mem, str, 50);
				continue;
			}

			// IO Port 0xE4 - HC_gvaToHva
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE4)
			{
				...
				// Make the kvm translate struct
				struct kvm_translation kvmTrans;
				kvmTrans.linear_address = *(uint32_t *)&vm->mem[gva];

				// Call KVM_TRANSLATE ioctl. (Translates gva to gpa)
				if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &kvmTrans) < 0)
				{
					perror("KVM Translate Error");
					exit(1);
				}
				...
				if (kvmTrans.valid)
				{
					// Get the guest physical address from struct
					uint64_t gpa = kvmTrans.physical_address;
					// Calculate the hva
					uint64_t hva = (uintptr_t)vm->mem + gpa;
					// Set the hva to the memory where gva was set
					*(uint32_t *)&vm->mem[gva] = (uint32_t)hva;
					continue;
				}
				printf("Invalid GVA\n");
				// If invalid place 0
				*(uint32_t *)&vm->mem[gva] = (uint32_t)0;
				continue;
			}
